...
- for the file browser I want something very minimal much like nerdtree in vim. Icons would be nice but are not necessary. Ranger seems to be the best fit for what I want. It is minimal (no icons and seems very fast) and it uses the vim keybindings so it plays very nicely with evil mode.
- rebinding the keys will make it a bit easier on the hands 

  | key-binding | function                                        |
  |-------------+-------------------------------------------------|
  | SPC-d       | from any mode this will summon ranger           |
  | q           | from a ranger buffer this will close the buffer |
  | //          | search                                          |
  | RET         | open file                                       |
  | l           | go left / open file                             |
  | h           | go right / back in directory tree               |
  | j           | go down                                         |
  | k           | go up                                           |
  | C-h         | show hidden directories / files                 |
  | ws          | exit and open selected file in horizontal split |
  | wv          | exit and open selected file in vertical split   |
  ...
  ;; set up a default binding 
  (general-define-key
   :states '(normal)
   :keymaps 'override
   :prefix "SPC"
   "d" '(ranger :which-key "open ranger"))
  ,#+end_src
