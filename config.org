...
;; ----- Fonts
(add-to-list 'default-frame-alist '(font . "Fira Code Nerd Font 22"))
...
(defun cpp--run-cmd (cmd &optional args)
...
(defun custcpp-build-file ()
  "Build go project"
  (interactive)
  (let* ((fname (buffer-file-name))
		 (len (length fname))
		 (name (substring fname 0 (- len 4))))
	(shell-command (concat "g++ -Wall " fname " -o " name))))
